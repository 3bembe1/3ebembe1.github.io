...
// В реальному проєкті URL береться зі змінних оточення (.env)
...
export const useActiveTasks = (userId, projectId) => {
  ...
  useEffect(() => {
    // Ініціалізація підключення з параметрами для бекенду
    const socket = io(SOCKET_SERVER_URL, {
      reconnectionAttempts: 5, // Кількість спроб перепідключення
      reconnectionDelay: 2000,
      query: { userId, projectId } // Передаємо ID, щоб бекенд підключив нас до правильної "кімнати"
    });

    // 1. Обробка успішного підключення
    socket.on('connect', () => {
      setIsConnected(true);
      setError(null);
    });
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (reason === 'io server disconnect') {
        // Якщо сервер примусово відключив, підключаємось заново
        socket.connect();
      }
    });

    // 3. Основна бізнес-логіка (US-52: Оновлення списку)
    socket.on('tasks:update', (updatedTasks) => {
      setTasks(updatedTasks);
    });

    // 4. Очищення підключення при знищенні компонента (Unmount)
    return () => {
      socket.off('connect');
  ...
  }, [userId, projectId]); // Перезапустити, якщо зміниться юзер або проєкт
...
const ActiveTasksBoard = ({ currentUser, currentProject }) => {
  // Використовуємо наш кастомний хук
